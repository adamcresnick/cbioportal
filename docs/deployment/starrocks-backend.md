...
StarRocks mode currently has concrete repositories for cancer studies, patients, samples, clinical attributes, clinical data, clinical events, treatments, mutation retrieval, alteration aggregates, and genomic-data aggregates. The generic-assay adapter currently implements only the molecular-profile discovery operations required by alteration enrichment; its data and metadata operations remain fail-fast pending the generic-assay port.

The implemented surface includes study metadata and resources, patient/sample counts, sample lists, clinical ID/summary/detailed projections, clinical counts and enrichment inputs, event counts, patient timeline projections, event-derived survival data, patient/sample treatment reports, mutation ID/summary/detailed/meta projections, mutation/CNA/structural-variant gene aggregates, profiled and gene-panel enrichment counts, molecular-profile sample counts, CNA and mutation chart counts, numeric genomic bins, and stable sorting and pagination. The Study View sample universe supports study, explicit sample, case-list, molecular-profile, clinical, event, patient-treatment, sample-treatment, gene, mutation, CNA/numeric molecular-value, and structural-variant selections. The public structural-variant fetch mapper uses portable bound list parameters for both MySQL and StarRocks.

Generic-assay, co-expression, custom-data, treatment-group, and treatment-target filter families remain owned by subsequent domain ports. Passing one of those filters through an implemented adapter raises `StarrocksUnsupportedStudyViewFilterException`; it is never silently ignored. Repository operations not yet backed by a concrete StarRocks implementation raise `StarrocksDomainNotImplementedException` with the repository and operation name.
