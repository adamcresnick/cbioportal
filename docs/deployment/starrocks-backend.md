...
StarRocks mode currently has concrete repositories for cancer studies, patients, samples, clinical attributes, clinical data, clinical events, treatments, and mutation retrieval. These adapters cover study metadata and resources, patient/sample counts, sample lists, clinical ID/summary/detailed projections, clinical counts and enrichment inputs, event counts, patient timeline projections, event-derived survival data, patient/sample treatment reports, mutation ID/summary/detailed/meta projections, stable sorting and pagination, and the Study View sample universe for study, explicit sample, case-list, molecular-profile, clinical, event, patient-treatment, and sample-treatment selections.

Gene, mutation-filtered Study View, CNA, structural-variant, molecular-value, generic-assay, co-expression, custom-data, treatment-group, and treatment-target filter families remain owned by subsequent domain ports. Passing one of those filters through an implemented adapter raises `StarrocksUnsupportedStudyViewFilterException`; it is never silently ignored. Repository interfaces not yet backed by a concrete StarRocks adapter continue to use the explicit not-implemented fallback until their domain port is merged.
